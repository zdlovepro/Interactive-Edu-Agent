...
public class UrlImportController {
    ...
    @PostMapping
    public BaseResponse<UrlImportTaskView> createTask(
            @Valid @RequestBody UrlImportRequest request,
            @RequestHeader(value = "Authorization", required = false) String authorizationHeader
    ) {
        String userId = authService.requireUser(authorizationHeader).id();
        return BaseResponse.ok(urlImportService.createTask(request.getUrl(), request.getName(), userId));
    }

    @GetMapping("/tasks/{taskId}")
    public BaseResponse<UrlImportTaskView> getTask(
            @PathVariable("taskId") @NotBlank String taskId,
            @RequestHeader(value = "Authorization", required = false) String authorizationHeader
    ) {
        String userId = authService.requireUser(authorizationHeader).id();
        return BaseResponse.ok(urlImportService.getTask(taskId, userId));
    }
}
